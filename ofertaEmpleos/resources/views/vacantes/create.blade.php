...
@section('content')
    ...
    <form action="{{route('vacantes.store')}}" method="post" class=" max-w-lg mx-auto my-10" novalidate >
        @csrf
        <div class="mb-5">
            <label for="" class="block text-gray-700 text-sm mb-2">Titulo vacante</label>
            <input
                id="titulo" type="text" class="p-3 bg-gray-100 rounded form-input w-full @error('titulo') border-red-500 border @enderror" name="titulo" value="{{ old('titulo') }}" required autocomplete="titulo" autofocus='autofocus'
        ...
        <div class="mb-5">
            ...
            <lista-habilidades
                    ...
                    :old_habilidades="{{ json_encode(old('habilidades'))}}"
                >
            </lista-habilidades>
            @error('habilidades')
                <div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 w-full mt-5 text-sm" role="alert">
